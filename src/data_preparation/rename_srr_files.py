...
for index, line in experiments.iterrows():
    ...
    key_tuple = tuple(donor, cell_type)
...
def encode_prefix(filename, celltype):
	''' take a fastq file encoded by its SRR id, re-encode it as 
	<donor id>_rep?_<original suffix>'''

	# SRR2920544_1.fastq.gz
	srr_id, suffix = filename.split("_")
	donor = srr_to_donor_id[srr_id]
	rep_number = donor_cell_type_rep_number[tuple(donor, celltype)].pop()
	new_name = celltype + "_" + donor + "_" + str(rep_number) + "_" + suffix 
	return new_name

# walk directories of data, renaming files as needed
for dirname, dirs, files in os.walk(os.path.expanduser("~/projects/SeqDemote/data/ATAC")):
