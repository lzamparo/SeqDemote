...
def validation_ap_objective(suggestion, model_module, model_name, trial_num, outfile=None):
    ...
    if outfile is not None:
        ...
        print(','.join([model_name, str(trial_num), 'AP', ap_str]), file=outfile, flush=True)
        f1_str = "{0:.4f}".format(max(losses_valid_f1) * 100)
        print(','.join([model_name, str(trial_num), 'F1', f1_str]), file=outfile, flush=True)
        mcc_str = "{0:.4f}".format(max(losses_valid_mcc) * 100)
        print(','.join([model_name, str(trial_num), 'MCC', mcc_str]), file=outfile, flush=True)
