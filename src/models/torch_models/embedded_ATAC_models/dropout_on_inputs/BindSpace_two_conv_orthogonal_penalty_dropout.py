...
model_hyperparams_dict={'first_filters': {'type': 'int', 'min': 20, 'max': 200},
                        'dropout': {'type': 'float', 'min': 0.05, 'max': 0.9},
...
class BindSpaceNet(nn.Module):
    ...
    def __init__(self, input_size=(1, 281, 300), num_factors=24, hyperparams_dict=default_hyperparams):
        ...
        self.relu = nn.SELU()
        self.dropout = nn.AlphaDropout(hyperparams_dict['dropout'])
    ...
    def forward(self, input):
        ...
        x = self.pool1(self.relu(self.orth_conv1(x)))
...
# Initialize the otimizer params, put together arguments for optimizer
