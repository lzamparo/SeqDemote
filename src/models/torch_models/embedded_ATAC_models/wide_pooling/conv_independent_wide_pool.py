...
class BindSpaceNet(nn.Module):
    ...
    def __init__(self, input_size=(1, 281, 300), num_factors=24, hyperparams_dict=default_hyperparams):
        ...
        self.fingers = nn.ModuleList([nn.Sequential(nn.utils.weight_norm(nn.Conv2d(1, num_filters, (1,300))),
                                    self.relu, nn.MaxPool2d(kernel_size=(10,1)),          
                                    nn.utils.weight_norm(nn.Conv2d(num_filters,10,(14,1))), 
                                    self.relu, nn.MaxPool2d((3,1)), nn.Dropout(p=0.25)) for f in range(num_factors)])
